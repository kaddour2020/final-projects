...
@section('title','المتجر العربي || تحرير الشعار')
@section('main-content')

<div class="card">
    <h5 class="card-header">تحرير الشعار</h5>
    <div class="card-body">
      <form method="post" action="{{route('banner.update',$banner->id)}}">
        ...
        <input id="inputTitle" type="text" name="title" placeholder="أدخل العنوان"  value="{{$banner->title}}" class="form-control">
        @error('title')
        ...
        <div class="form-group">
          ...
          <select name="status" class="form-control">
            <option value="active" {{(($banner->status=='active') ? 'selected' : '')}}>نشيط</option>
            <option value="inactive" {{(($banner->status=='inactive') ? 'selected' : '')}}>غير نشط</option>
          </select>
          @error('status')
          <span class="text-danger">{{$message}}</span>
          @enderror
