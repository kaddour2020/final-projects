...
@section('title','المتجر العربي || تحرير العلامة التجارية')
@section('main-content')

<div class="card">
    <h5 class="card-header">تحرير العلامة التجارية</h5>
    <div class="card-body">
      <form method="post" action="{{route('brand.update',$brand->id)}}">
        ...
        <div class="form-group">
          ...
          <select name="status" class="form-control">
            <option value="active" {{(($brand->status=='active') ? 'selected' : '')}}>نشيط</option>
            <option value="inactive" {{(($brand->status=='inactive') ? 'selected' : '')}}>غير نشط</option>
          </select>
          @error('status')
          <span class="text-danger">{{$message}}</span>
          @enderror
